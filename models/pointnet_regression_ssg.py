...
#     def __init__(self,num_class,coordinates=True):
#         super(get_model, self).__init__()
#         in_channel = 6 if coordinates else 3
...
class get_model(nn.Module):
    def __init__(self, num_class, coordinates=True, feat_dim=3):
        """
        coordinates=True  : input is [B, P, 3 + feat_dim], use first 3 as xyz, rest as features
        coordinates=False : input is [B, P, 3] OR [B, P, feat_dim]; we still treat first 3 channels as xyz,
                            and do not pass extra features (points=None) to SA layers.
        """
        super(get_model, self).__init__()
        self.coordinates = coordinates
        self.feat_dim = int(feat_dim)

        # points(feature) channels passed into SA1
        in_channel = (3 + self.feat_dim) if self.coordinates else 3

        # msg (keep your architecture)
        self.sa1 = PointNetSetAbstractionMsg(
            npoint=1024,
            radius_list=[0.1, 0.2, 0.3],
            nsample_list=[32, 48, 64],
            in_channel=in_channel,
            mlp_list=[[32, 64, 128], [64, 96, 192], [64, 128, 256]],
        )
        self.sa2 = PointNetSetAbstractionMsg(
            npoint=256,
            radius_list=[0.2, 0.4, 0.5],
            nsample_list=[64, 96, 128],
            in_channel=(128 + 192 + 256) + 3,
            mlp_list=[[128, 128, 256], [128, 192, 256], [128, 256, 256]],
        )
        self.sa3 = PointNetSetAbstraction(
            npoint=None,
            radius=None,
    ...
    def forward(self, x):
        ...
        if C < 3:
            raise ValueError(f"Input channel C must be >=3 (xyz), got C={C}")

        xyz = x[:, :3, :]  # always required
        points = None
        if self.coordinates:
            if C <= 3:
                raise ValueError(f"coordinates=True requires C>3, got C={C}")
            points = x[:, 3:, :]  # [B, feat_dim, P]

        l1_xyz, l1_points = self.sa1(xyz, points)
        l2_xyz, l2_points = self.sa2(l1_xyz, l1_points)
        l3_xyz, l3_points = self.sa3(l2_xyz, l2_points)

        out = l3_points.view(B, 2048)
...
class get_loss(nn.Module):
    """
    3-class PID loss (nc=0, Vmu=1, Ve=2)
    pred: logits [B, 3]
    target: class idx [B] (long)
    """
